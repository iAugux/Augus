...
public struct GeminiOAuthCreds: Codable, Sendable {
...
public struct GeminiModelQuota: Codable, Sendable, Identifiable {
...
public struct GeminiUsageData: Codable, Sendable {
    public let models: [GeminiModelQuota]
    public let lastUpdated: Date
    public let email: String
    
    public init(models: [GeminiModelQuota], lastUpdated: Date, email: String) {
        self.models = models
...
public class GeminiStore {
    ...
    public static func saveOAuthCreds(_ creds: GeminiOAuthCreds) {
        if let encoded = try? JSONEncoder().encode(creds) {
            defaults.set(encoded, forKey: "gemini_oauth_creds")
            defaults.synchronize()
            print("GeminiStore: Saved OAuth credentials.")
        }
    }
    
    public static func loadOAuthCreds() -> GeminiOAuthCreds? {
        guard let data = defaults.data(forKey: "gemini_oauth_creds") else { return nil }
        return try? JSONDecoder().decode(GeminiOAuthCreds.self, from: data)
    }
    
    public static func saveUsageData(_ data: GeminiUsageData) {
        if let encoded = try? JSONEncoder().encode(data) {
            defaults.set(encoded, forKey: "gemini_usage_data")
            defaults.synchronize()
            print("GeminiStore: Saved usage data.")
        }
    }
    
    public static func loadUsageData() -> GeminiUsageData? {
        guard let data = defaults.data(forKey: "gemini_usage_data") else { return nil }
        return try? JSONDecoder().decode(GeminiUsageData.self, from: data)
    }
    
    public static func saveLastLog(_ log: String) {
        defaults.set(log, forKey: "gemini_last_log")
        defaults.synchronize()
    }
    
    public static func loadLastLog() -> String {
        return defaults.string(forKey: "gemini_last_log") ?? "No Gemini logs recorded yet."
    }
    
    public static func clear() {
        defaults.removeObject(forKey: "gemini_oauth_creds")
        defaults.removeObject(forKey: "gemini_usage_data")
        defaults.removeObject(forKey: "gemini_last_log")
        defaults.synchronize()
        print("GeminiStore: Cleared all stored data.")
        WidgetCenter.shared.reloadAllTimelines()
    }
}

// MARK: - Gemini Network Manager

public final class GeminiNetworkManager: Sendable {
    public static let shared = GeminiNetworkManager()
    
    private init() {}
    
    // Google Client ID and Client Secret extracted from Gemini CLI (Code Assist)
    ...
    public func fetchUsage(completion: @escaping @Sendable (Result<GeminiUsageData, Error>) -> Void) {
        guard let creds = GeminiStore.loadOAuthCreds() else {
            let error = NSError(domain: "GeminiNetworkManager", code: -1, userInfo: [NSLocalizedDescriptionKey: "No Gemini OAuth credentials found. Please paste your token JSON in the App first."])
        ...
        if currentMs + bufferMs >= creds.expiryDate {
            GeminiStore.saveLastLog("Access token expired or expiring soon. Refreshing token...")
    ...
    private func refreshAccessToken(creds: GeminiOAuthCreds, completion: @escaping @Sendable (Result<GeminiOAuthCreds, Error>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard let data = data else {
                completion(.failure(NSError(domain: "GeminiNetworkManager", code: -2, userInfo: [NSLocalizedDescriptionKey: "No data received during token refresh."])))
                return
            }
            ...
            do {
                ...
                GeminiStore.saveOAuthCreds(updated)
                GeminiStore.saveLastLog("Successfully refreshed Google OAuth token.")
                ...
                GeminiStore.saveLastLog(logMsg)
                completion(.failure(error))
            }
        }
        task.resume()
    ...
    private func fetchUsageWithToken(_ accessToken: String, completion: @escaping @Sendable (Result<GeminiUsageData, Error>) -> Void) {
            ...
            case .success(let codeAssistData):
                ...
                self.retrieveUserQuota(accessToken: accessToken, projectId: projectId) { quotaResult in
                    ...
                    case .success(let models):
                        let usageData = GeminiUsageData(models: models, lastUpdated: Date(), email: email)
                        GeminiStore.saveUsageData(usageData)
                        GeminiStore.saveLastLog("Successfully updated Gemini quotas for \(email). Found \(models.count) model buckets.")
                        WidgetCenter.shared.reloadAllTimelines()
    ...
    private func loadCodeAssist(accessToken: String, completion: @escaping @Sendable (Result<CodeAssistInfo, Error>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard let data = data else {
                ...
                GeminiStore.saveLastLog(log)
                completion(.failure(NSError(domain: "GeminiNetworkManager", code: -3, userInfo: [NSLocalizedDescriptionKey: log])))
                return
            }
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                    guard let projectId = json["cloudaicompanionProject"] as? String, !projectId.isEmpty else {
                        ...
                        GeminiStore.saveLastLog(log)
                        completion(.failure(NSError(domain: "GeminiNetworkManager", code: -4, userInfo: [NSLocalizedDescriptionKey: "Could not retrieve bound Google Cloud project ID."])))
                        return
                    }
                    ...
                    GeminiStore.saveLastLog(log)
                    completion(.failure(NSError(domain: "GeminiNetworkManager", code: -5, userInfo: [NSLocalizedDescriptionKey: log])))
                }
            } catch {
    ...
    private func retrieveUserQuota(accessToken: String, projectId: String, completion: @escaping @Sendable (Result<[GeminiModelQuota], Error>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard let data = data else {
                ...
                GeminiStore.saveLastLog(log)
                completion(.failure(NSError(domain: "GeminiNetworkManager", code: -6, userInfo: [NSLocalizedDescriptionKey: log])))
                return
            }
            ...
            do {
                ...
                if let buckets = responseObj.buckets {
                    for bucket in buckets {
                        ...
                        models.append(GeminiModelQuota(name: modelId, remainingFraction: fraction, resetTime: resetDate))
                    }
                }
